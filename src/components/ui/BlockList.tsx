...
// Server-safe renderer for the shared block-editor content model. Used by:
//   - TourBody (client wrapper, adds a departure-city filter)
//   - Destination page (server, no city filter needed)
// If you add a new block variant, extend the switch below AND update the
// admin BlockEditor's BlockFields component.
export function BlockList({ blocks }: { blocks: TourBlock[] }) {
...
export function BlockRenderer({ block }: { block: TourBlock }) {
  switch (block.type) {
    case "heading": {
      if (block.level === 2) {
