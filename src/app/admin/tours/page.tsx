...
import { duplicateTour, deleteTour, setTourPublished } from "./actions";
...
async function fetchTours(): Promise<TourRowLite[]> {
  ...
  const { data, error } = await supabase
    .from("tours")
    .select("slug, name, destination_slug, region_slug, duration, category, anchor_city, published")
    .order("name");
...
export default async function AdminToursPage() {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      ...
      <ToursListClient rows={rows} duplicateAction={duplicateTour} deleteAction={deleteTour} setPublishedAction={setTourPublished} />
    </div>
  );
}
