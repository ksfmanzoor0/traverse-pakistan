...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data: depRow, error: depErr } = await supabase
      ...
      .maybeSingle();
    if (depErr || !depRow) {
      console.error("[api/tours/create-booking] departure not found:", body.departureId, depErr);
    ...
    if (!quote) {
      console.error("[api/tours/create-booking] no quote for tour:", dep.tour_slug, body.homeCity);
    ...
    if (!result) {
      console.error("[api/tours/create-booking] rpc returned empty:", data);
    ...
    console.error("[api/tours/create-booking] uncaught:", message, stack);
