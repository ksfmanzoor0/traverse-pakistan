...
export async function POST(req: NextRequest, ctx: { params: Promise<{ slug: string }> }) {
  ...
  if (file.size > MAX_BYTES) return NextResponse.json({ error: `File exceeds ${MAX_BYTES / 1024 / 1024} MB limit` }, { status: 400 });

  const bytes = new Uint8Array(await file.arrayBuffer());
  const key = `packages/${slug}/${crypto.randomUUID()}.${extForType(file.type)}`;
