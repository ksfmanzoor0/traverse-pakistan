...
/**
 * Returns the ORIGINAL as-booked PDF for a package booking. Reads only
 * itinerary_original — never touches itinerary_snapshot (the admin-editable
 * one). If itinerary_original is missing on an old booking, falls back to
 * the standard package itinerary as it exists now (best-effort).
 */
export async function GET(_req: Request, { params }: { params: Promise<{ ref: string }> }) {
  ...
  type BookingRow = {
    ...
    itinerary_original: PackageBookingSnapshot | null;
  };
  const { data, error } = await supabase
    .from("package_bookings")
    .select("booking_ref, package_slug, tier, departure_city, start_date, adults, rooms, contact_name, itinerary_original")
    .eq("booking_ref", ref)
    .maybeSingle();
  ...
  const booking: PdfBookingContext = {
    ...
    adults: row.adults ?? 1,
    rooms: row.rooms ?? 1,
    // If itinerary_original is missing on an older booking, snapshot stays
    // null and the renderer falls back to standard package data.
    snapshot: row.itinerary_original ?? null,
  };
  ...
  return new NextResponse(new Uint8Array(pdf), {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${safeName}-${row.booking_ref}-Original.pdf"`,
